...
def get_adress():
    cursorr.execute("SELECT id from Etudiant where cne='"+DataBase.get_cne_user_connecter()+"';")
    id=cursorr.fetchone()
    print(id)
    ...
    return username
def get_user_CNE():
    cursorr.execute("SELECT CNE from Etudiant where Cne='"+DataBase.get_cne_user_connecter()+"';")
    result=cursorr.fetchone()
    cne=result[0]
    return cne

def get_user_CIN():
    cursorr.execute("SELECT CIN from Etudiant where Cne='"+DataBase.get_cne_user_connecter()+"';")
    result=cursorr.fetchone()
    cin=result[0]
    return cin
